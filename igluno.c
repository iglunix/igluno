...
static char rcsid[] = "$Id: igluno.c 920 2008-01-31 00:44:12Z hubert@u.washington.edu $";
#endif
 ...
 *	       functions added.
 *
 * 09 Sep 91 - weemacs() renamed igluno() for the PIne COmposer.
 *
 */
...
/*
 * flag for the various functions in igluno() to set when ready
 * for igluno() to return...
 */
int      igluno_all_done = 0;
jmp_buf  finstate;
UCS     *igluno_anchor = NULL;
extern struct headerentry *headents;

/*
 * igluno - the main routine for Pine's composer.
 *
 */
int
igluno(IGLUNO *pm)
{
    ...
    if(Pmaster->pine_anchor)
      igluno_anchor = utf8_to_ucs4_cpystr(Pmaster->pine_anchor);
    else
      igluno_anchor = NULL;
    ...
    igluno_all_done = setjmp(finstate);	/* jump out of HUP handler ? */

    if(gmode & MDALTNOW){
	while(!igluno_all_done){
	    if(((gmode & P_BODY) || !Pmaster->headents)
	       && alt_editor(0, 1) < 0)
	      break;			/* if problem, drop into igluno */
		...
		gmode |= P_BODY;	/* make sure we enter alt ed next */
	      ...
	      igluno_all_done = COMP_EXIT;
	}
    }
    else if(!igluno_all_done){
	...
	if(igluno_all_done){
	    ...
	    switch(igluno_all_done){	/* prepare for/handle final events */
...
execute(UCS c, int f, int n)
{
	...
	/* igluno never does C mode, this is simple */
	status = linsert(n, c);
...
abort_composer(int f, int n)
{
    ...
    if(Pmaster->canceltest){
	  ...
	  || (result = (*Pmaster->canceltest)(redraw_igluno_for_callback))){
	    igluno_all_done = COMP_CANCEL;
...
/*
 * igluno_help - help function for standalone composer
 * Oops - looks like utf8title is unused!
 *
 * This should be fixed to handle TAB characters.
 */
int
igluno_help(char *utf8text[], char *utf8title, int i)
{
 ...
 *
 * If the mouse moves above or below the igluno body region of the
 ...
 */
unsigned long
mouse_in_igluno(unsigned long mevent, int row, int col, int button, int flags)
{
	...
	/* Ignore mouse down if not in igluno body region. */
...
doubleclick = (lrow == row && lcol == col
	...
	if (row < (Pmaster ? ComposerTopLine : 2)) {
	    ...
	    register_mtrack (mouse_in_igluno);
	    update ();
...
/*
 * Below are functions for use outside igluno to manipulate text
 * in a igluno's native format (circular linked list of lines).
 *
 * The idea is to streamline igluno use by making it fairly easy
 * for outside programs to prepare text intended for igluno's use.
...
#define PT(X)	((iglunoTEXT *)(X))

/*
 * igluno_get - return window struct pointer used as a handle
 *            to the other igluno_xxx routines.
 */
void *
igluno_get(void)
{
   ...
   if((wp = (iglunoTEXT *)malloc(sizeof(iglunoTEXT))) != NULL){
       wp->crinread = 0;
       if((lp = lalloc(0)) == NULL){
	   free(wp);
	   return(NULL);
...
/*
 * igluno_give - free resources and give up iglunotext struct
 */
void
igluno_give(void *w)
{
...
/*
 * igluno_writec - write a char into iglunotext and advance pointers.
 ...
 */
int
igluno_writec(void *w, int c, int flags)
{
    int   rv = 0;
...
/*
 * igluno_puts - just write the given string into the text
 */
int
igluno_puts(void *w, char *s, int flags)
{
    int rv = 0;
	...
	rv = 1;
	while(rv && *s != '\0')
	  if(!igluno_writec(w, (int)*s++, flags))
	    rv = 0;
    }
...
/*
 * igluno_seek - position dotp and dot at requested location
 */
int
igluno_seek(void *w, long offset, int orig)
{
    register LINE *lp;
...
int
igluno_cursor(int col, long row)
{
