 ...
 *              Note: home is malloc'd for life of igluno
 */
char *
gethomedir(int *l)
{
...
/*
 * pfnexpand - igluno's function to expand the given file name if there is 
...
tmpname(char *dir, char *name)
{
    char *t;
#ifndef _WINDOWS
    if((t = temp_nam((dir && *dir) ? dir : NULL, "igluno.")) != NULL){
#else /* _WINDOWS */
    char  tmp[_MAX_PATH];
...
#define	MODE_READONLY	(0600)
    ...
    g_igluno_fio.flags = FIOINFO_WRITE;
    g_igluno_fio.name  = fn;
#ifndef _WINDOWS
    if((fd = our_open(fn, O_CREAT|O_WRONLY, readonly ? MODE_READONLY : 0666)) >= 0
       && (g_igluno_fio.fp = fdopen(fd, "w")) != NULL
       && fseek(g_igluno_fio.fp, 0L, 0) == 0)
      return (FIOSUC);
    ...
    return (FIOERR);
#else /* _WINDOWS */
    if ((g_igluno_fio.fp = our_fopen(fn, "w")) == NULL) {
...
ffclose(void)
{
    ...
    errno = 0;
    if((g_igluno_fio.flags & FIOINFO_WRITE)
       && (fflush(g_igluno_fio.fp) == EOF
	   || ftruncate(fileno(g_igluno_fio.fp),
			(off_t) ftell(g_igluno_fio.fp)) < 0)){
	eml.s = errstr(errno);
	emlwrite("\007Error preparing to close file: %s", &eml);
	sleep(5);
    }

    if (fclose(g_igluno_fio.fp) == EOF) {
	eml.s = errstr(errno);
        emlwrite("\007Error closing file: %s", &eml);
        return(FIOERR);
    }
#else /* _WINDOWS */
    if (fclose(g_igluno_fio.fp) != FALSE) {
...
ffelbowroom(void)
{
    ...
    if(fstat(fileno(g_igluno_fio.fp), &fsbuf) == 0){
	n -= fsbuf.st_size;

	if(n > 0L){			/* must be growing file, extend it */
	    memset(s, 'U', EXTEND_BLOCK);
	    if(fseek(g_igluno_fio.fp, fsbuf.st_size, 0) == 0){
		for( ; n > 0L; n -= EXTEND_BLOCK){
		    x = (n < EXTEND_BLOCK) ? (int) n : EXTEND_BLOCK;
		    if(fwrite(s, x * sizeof(char), 1, g_igluno_fio.fp) != 1){
		...
		if(!errstring
		   && (fflush(g_igluno_fio.fp) == EOF
		       || fsync(fileno(g_igluno_fio.fp)) < 0))
		  errstring = errstr(errno);

		if(errstring)			/* clean up */
		  (void) ftruncate(fileno(g_igluno_fio.fp), (off_t) fsbuf.st_size);
		else if(fseek(g_igluno_fio.fp, 0L, 0) != 0)
		  errstring = errstr(errno);
	    }
	    else
	      errstring = errstr(errno);
    ...
    if(errstring){
	snprintf(s, sizeof(s), "Error writing to %s: %s", g_igluno_fio.name, errstring);
	emlwrite(s, NULL);
	(void) fclose(g_igluno_fio.fp);
	return(FALSE);
    }
#endif /* UNIX */
    return(TRUE);
