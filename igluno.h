/*
 * $Id: igluno.h 1165 2008-08-22 20:40:16Z jpf@u.washington.edu $
 *
 * ========================================================================
 ...
 * ========================================================================
 *
 * Program:	igluno.h - definitions for Pine's composer library
 */
 ...
 * igluno is called. The list is terminated by an entry with the name
 * element NULL.
 */
...
typedef struct igluno_atmt {
    ...
    struct igluno_atmt *next;
} PATMT;

/*
 * Structure to contain color options
 */
typedef struct igluno_colors {
...
/*
 * Used to save and restore global igluno variables that are destroyed by
 * calling igluno a second time. This happens when igluno calls a selector
 * in the HeaderEditor and that selector calls igluno again.
 */
typedef struct save_stuff {
    ...
    int                 igluno_all_done;
    jmp_buf             finstate;
    UCS                *igluno_anchor;	/* save pointer */
    IGLUNO               *Pmaster;	/* save pointer */
    int                 fillcol;
    UCS                *pat;		/* save array */
    int                 ComposerTopLine,
                        ComposerEditing;
...
/*
 * various flags that they may passed to igluno
 */
 ...
 */
#ifdef	maindef
IGLUNO	*Pmaster = NULL;		/* composer specific stuff */
char	*version = "0.01";		/* igluno version number */

#else
extern	IGLUNO *Pmaster;			/* composer specific stuff */
extern	char *version;			/* igluno version! */
...
/*
 * Flags for igluno_readc/igluno_writec.
 */
#define IGLUNOREADC_NONE	0x00
#define IGLUNOREADC_NOUCS	0x01
 ...
 */
int   igluno(IGLUNO *pm);
int   igluno_file_browse(IGLUNO *, char *, size_t, char *, size_t, char *, size_t, int);
void *igluno_get(void);
void  igluno_give(void *w);
int   igluno_readc(void *w, unsigned char *c, int flags);
int   igluno_writec(void *w, int c, int flags);
int   igluno_puts(void *w, char *s, int flags);
int   igluno_seek(void *w, long offset, int orig);
int   igluno_replace(void *, char *);
int   igluno_fncomplete(char *, char *, size_t);
#if defined(DOS) || defined(OS2)
int   igluno_nfsetcolor(char *);
int   igluno_nbsetcolor(char *);
int   igluno_rfsetcolor(char *);
int   igluno_rbsetcolor(char *);
#endif
#ifdef	MOUSE
...
unsigned long mouse_in_igluno(unsigned long, int, int, int, int);
